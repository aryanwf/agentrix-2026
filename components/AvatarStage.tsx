...
export default function AvatarStage({ ref, onReady, onError, className, options }: Props) {
        ...
        loadTalkingHead(node, { options, onProgress: (f) => !cancelled && setProgress(f) })
            ...
            if (process.env.NODE_ENV !== "production") {
                (window as unknown as {
                    __cura_head?: TalkingHead;
                }).__cura_head = instance;
            }
            setStatus("ready");
            onReadyRef.current?.();
        })
    ...
    useImperativeHandle(ref, (): AvatarHandle => ({
        ...
        speak: async (text, opts) => {
            ...
            if (opts?.mute) {
                ...
                if (opts?.onPlayed)
                    head.speakMarker(opts.onPlayed);
    ...
    return (<div className={`relative ${className ?? ""}`}>
      ...
      {status !== "ready" && (<div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          {status === "loading" ? (<div className="flex flex-col items-center gap-3 text-sm text-zinc-400">
              <div className="h-1 w-40 overflow-hidden rounded-full bg-zinc-800">
                <div className="h-full rounded-full bg-emerald-500 transition-[width] duration-200" style={{ width: `${Math.round(progress * 100)}%` }}/>
              </div>
              <span>loading avatar… {Math.round(progress * 100)}%</span>
            </div>) : (<div className="max-w-sm rounded-lg border border-red-900/60 bg-red-950/40 p-4 text-sm text-red-200">
