...
export default function Studio3D() {
  ...
  useEffect(() => {
    ...
    const probe = async () => {
      try {
        ...
        append(
          next.configured
            ? `tts: elevenlabs ready${
                next.quota ? ` — ${next.quota.remaining.toLocaleString()} credits left` : ""
              }`
            : "tts: ELEVENLABS_API_KEY missing — silent lip-sync preview",
        );
        if (next.quotaError) append(`tts: quota unknown (${next.quotaError})`);
        ...
        if (!controller.signal.aborted) setVoices(data.voices ?? []);
      } catch (err) {
        if (!controller.signal.aborted) append(`tts: probe failed (${(err as Error).message})`);
      }
    };
  ...
  const handleError = useCallback(
    (message: string) => append(`AVATAR FAILED: ${message}`),
    [append],
  );
    ...
    try {
      ...
      for (const sentence of sentences) {
        ...
        if (result?.mode === "audio") {
          if (!result.cached) billed += result.chars;
          append(
            `  ✓ ${result.words} words, ${result.chars} chars${result.cached ? " (cached, free)" : ""}`,
          );
        }
      }
      if (billed > 0) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col bg-[#12161c] text-zinc-100 lg:flex-row">
      ...
      <aside className="flex w-full shrink-0 flex-col gap-3 border-t border-zinc-800 bg-[#0d1117] p-5 lg:w-[360px] lg:border-l lg:border-t-0">
        ...
        {status?.quota && (
          <div className="rounded-md border border-zinc-800 bg-[#161b22] p-2 text-xs text-zinc-400">
            ...
            <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-zinc-800">
              <div
                className={`h-full rounded-full ${
                  status.quota.remaining / status.quota.limit < 0.2
                    ? "bg-red-500"
                    : "bg-emerald-500"
                }`}
                style={{
        ...
        {voices.length > 0 && (
          <label className="flex flex-col gap-1 text-xs text-zinc-400">
            ...
            <select
              ...
              className="rounded-md border border-zinc-700 bg-[#161b22] p-2 text-sm text-zinc-100 outline-none focus:border-emerald-600"
            >
              {voices.map((v) => (
        ...
        <textarea
          ...
          rows={4}
          className="w-full resize-y rounded-md border border-zinc-700 bg-[#161b22] p-2 text-sm outline-none focus:border-emerald-600"
        />
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 rounded-md bg-emerald-600 px-3 py-2 text-sm font-semibold text-white transition-colors hover:bg-emerald-500 disabled:cursor-not-allowed disabled:bg-zinc-700 disabled:text-zinc-400"
          >
          ...
          <button
            onClick={stop}
            disabled={!ready}
            className="rounded-md border border-zinc-700 px-3 py-2 text-sm transition-colors hover:bg-zinc-800 disabled:cursor-not-allowed disabled:text-zinc-500"
          >
            Stop
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {GESTURES.map((name) => (
            <button
              key={name}
              onClick={() => gesture(name)}
              disabled={!ready}
              className="rounded-md border border-zinc-700 px-2.5 py-1.5 text-xs transition-colors hover:bg-zinc-800 disabled:cursor-not-allowed disabled:text-zinc-500"
            >
              {name}
            </button>
          ))}
