...
export async function POST(req: Request) {
    ...
    try {
        upstream = await fetch(`${API_BASE}/text-to-speech/${encodeURIComponent(voiceId)}/with-timestamps?output_format=${format}`, {
            method: "POST",
    ...
    if (!upstream.ok) {
        ...
        return json({ error: `ElevenLabs returned ${upstream.status}`, detail: detail.slice(0, 500) }, upstream.status === 401 || upstream.status === 429 ? upstream.status : 502);
    }
