import { convertToModelMessages, createUIMessageStream, createUIMessageStreamResponse, streamText, toUIMessageStream, type UIMessage, } from "ai";
...
import { API_BASE_URL, attributionHeaders, chatModels, OpenRouterError, requireKey, } from "@/lib/openrouter";
...
function normalizeMessages(input: UIMessage[]): UIMessage[] {
    return input
        ...
        message.parts.every((part) => !!part && part.type === "text" && typeof part.text === "string"))
        .slice(-MAX_MESSAGES)
        .map((message) => ({
        ...message,
...
function isTextPart(part: UIMessage["parts"][number]): part is Extract<UIMessage["parts"][number], {
    type: "text";
}> {
